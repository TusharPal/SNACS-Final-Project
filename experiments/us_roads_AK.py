...
us_roads_AK_df = pd.read_csv(glob.glob("../data/us_roads_AK/edges.csv")[0], on_bad_lines="skip")
us_roads_AK_df["source"] = us_roads_AK_df["# source"].astype(int)
us_roads_AK_df["target"] = us_roads_AK_df[" target"].astype(int)

print(us_roads_AK_df.head())
...
for row in us_roads_AK_df[["source", "target"]].to_records(index=False).tolist():
...
results = {
            "dataset": "us_roads_AK",
            "directed": False,
            "nodes": len(set(us_roads_AK_df["source"].to_list() + us_roads_AK_df["target"].to_list())),
