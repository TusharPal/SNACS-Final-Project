...
email_enron_df = pd.read_csv("../data/email_enron/edges.csv", on_bad_lines="skip")

email_enron_df["source"] = email_enron_df["# source"].astype(int)
email_enron_df["target"] = email_enron_df[" target"].astype(int)

print(email_enron_df.head())
...
for row in email_enron_df[["source", "target"]].to_records(index=False).tolist():
...
results = {
            "dataset": "email_enron",
            "directed": False,
            "nodes": len(set(email_enron_df["source"].to_list() + email_enron_df["target"].to_list())),
