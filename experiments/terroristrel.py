...
terroristrel_df = pd.read_csv("../data/terroristrel/TerroristRel.edges", sep=",", header=None, index_col=None, names=["source", "target"])

terroristrel_df["source"] = terroristrel_df["source"].astype(int)
terroristrel_df["target"] = terroristrel_df["target"].astype(int)

print(terroristrel_df.head())
...
for row in terroristrel_df[["source", "target"]].to_records(index=False).tolist():
...
results = {
            ...
            "directed": False,
            "nodes": len(set(terroristrel_df["source"].to_list() + terroristrel_df["target"].to_list())),
