...
digg_reply_df = pd.read_csv("../data/digg_reply/edges.csv", on_bad_lines="skip")

digg_reply_df["source"] = digg_reply_df["# source"].astype(int)
digg_reply_df["target"] = digg_reply_df[" target"].astype(int)

print(digg_reply_df.head())
...
for row in digg_reply_df[["source", "target"]].to_records(index=False).tolist():
...
results = {
            "dataset": "digg_reply",
            "directed": False,
            "nodes": len(set(digg_reply_df["source"].to_list() + digg_reply_df["target"].to_list())),
