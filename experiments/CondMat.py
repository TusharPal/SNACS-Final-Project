...
CondMat_df = pd.read_csv("../data/CondMat/ca-CondMat.txt", sep="\t", on_bad_lines="skip", header=None, names=["source", "target"])[4:]

CondMat_df["source"] = CondMat_df["source"].astype(int)
CondMat_df["target"] = CondMat_df["target"].astype(int)
...
for row in CondMat_df[["source", "target"]].to_records(index=False).tolist():
...
results = {
            "dataset": "CondMat",
            "directed": False,
            "nodes": len(set(CondMat_df["source"].to_list() + CondMat_df["target"].to_list())),
