...
with open('../data/openflights/inf-openflights.edges') as f:
    next(f)
    next(f)
    
    openflights_df = pd.DataFrame([line.split("\n")[0].split(" ")[:2] for line in f.readlines()], columns=["source", "target"])
    
openflights_df["source"] = openflights_df["source"].astype(int)
openflights_df["target"] = openflights_df["target"].astype(int)

print(openflights_df.head())
...
results = {
            "dataset": "AstroPh",
            "directed": False,
            "nodes": len(set(openflights_df["source"].to_list() + openflights_df["target"].to_list())),
