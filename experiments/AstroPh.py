...
AstroPh_df = pd.read_csv("../data/AstroPh/ca-AstroPh.txt", sep="\t", on_bad_lines="skip", header=None, names=["source", "target"])[4:]

AstroPh_df["source"] = AstroPh_df["source"].astype(int)
AstroPh_df["target"] = AstroPh_df["target"].astype(int)

print(AstroPh_df.head())
...
results = {
            "dataset": "AstroPh",
            "directed": False,
            "nodes": len(set(AstroPh_df["source"].to_list() + AstroPh_df["target"].to_list())),
